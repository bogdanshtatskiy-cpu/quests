...
export const BQ = {
    ...
    // Поиск значения NBT игнорируя суффиксы типов (:8, :10 и т.д.)
    getRawValue(obj, keyPrefix) {
        if (!obj || typeof obj !== 'object') return null;
        for (let k in obj) {
            if (k === keyPrefix || k.startsWith(keyPrefix + ':')) {
                return obj[k];
            }
        }
        return null;
    ...
    // ГЛАВНАЯ ФУНКЦИЯ: Формирование кастомного имени по NBT
    getCustomName(foundItem, tag) {
        let name = foundItem.name; 

        if (!tag) return name;

        // 1. ПРОВЕРКА ПЕРЕИМЕНОВАНИЯ (display -> Name)
        const display = this.getRawValue(tag, 'display');
        if (display) {
            const customName = this.getRawValue(display, 'Name');
            if (customName) {
                // Сохраняем форматирование (§), убираем только артефакты 'ğ'
                name = customName.trim().replace(/ğ/g, '');
            }
        }

        // 2. ПРОВЕРКА ЗАЧАРОВАНИЙ (ench или StoredEnchantments)
        ...
        if (enchs && typeof enchs === 'object') {
            ...
            if (firstEnch) {
                ...
                if (foundItem.item_key.includes('enchanted_book')) {
                    name = `§eКнига:§r ${enchTitle} ${roman}`;
                } else {
                    name += ` §7[${enchTitle} ${roman}]§r`;
                }
            }
        }

        // 3. ПРОВЕРКА СОДЕРЖИМОГО (Для тайников/сейфов)
    ...
    parseLootData(jsonString, editor) {
        try {
            ...
            console.error("Ошибка парсинга лутбоксов:", e);
        }
    },
        ...
        try {
            ...
            if (data.questDatabase) {
                Object.entries(data.questDatabase).forEach(([qKey, q]) => {
                            ...
                            if (task.requiredItems && (task.taskID === 'bq_standard:retrieval' || task.taskID === 'bq_standard:crafting')) {
                                Object.values(task.requiredItems).forEach(item => {
                                    ...
                                    reqs.push({ 
                                        item: foundItem, 
                                        count: item.Count || 1, 
                                        customName: this.getCustomName(foundItem, item.tag), 
                                        consume: task.consume || false, 
                                        taskType: task.taskID.split(':')[1], 
                                        nbtTag: item.tag 
                                    });
                                });
                                ...
                                Object.values(task.blocks).forEach(block => {
                                    ...
                                    reqs.push({ 
                                        item: foundItem, 
                                        count: block.amount || block.Count || 1, 
                                        customName: this.getCustomName(foundItem, block.nbt), 
                                        consume: false, 
                                        taskType: 'block_break', 
                                        nbtTag: block.nbt 
                                    });
                                });
                                 ...
                                 reqs.push({ 
                                     item: { item_key: `mob_${target}`, name: target, image: 'skull.png', mod: 'Мобы' }, 
                                     count: task.required || 1, 
                                     target: target, 
                                     customName: target, 
                                     consume: false, 
                                     taskType: 'hunt', 
                                     nbtTag: task.targetNBT 
                                 });
                            } else if (task.taskID === 'bq_standard:fluid') {
                                Object.values(task.requiredFluids || {}).forEach(fluid => {
                                    const fname = fluid.FluidName || "water";
                                    reqs.push({ 
                                        item: { item_key: `fluid_${fname}`, name: this.FLUIDS[fname] || fname, image: 'fluid_bucket.png', mod: 'Жидкость' }, 
                                        count: fluid.Amount || 1000, 
                                        target: fname, 
                                        customName: this.FLUIDS[fname] || fname, 
                                        consume: task.consume || false, 
                                        taskType: 'fluid' 
                                    });
                                });
                                ...
                                reqs.push({ 
                                    item: { item_key: 'checkbox', name: 'Галочка', image: 'checkbox.png', mod: 'Задачи' }, 
                                    count: 1, 
                                    customName: 'Нажать галочку', 
                                    consume: false, 
                                    taskType: 'checkbox' 
                                });
                            } else if (task.taskID === 'bq_standard:xp') {
                                reqs.push({ 
                                    item: { item_key: 'xp', name: 'Опыт', image: 'experience_bottle.png', mod: 'Система' }, 
                                    count: task.amount || 1, 
                                    customName: 'Уровни опыта', 
                                    consume: task.consume || false, 
                                    taskType: 'xp' 
                                });
                                ...
                                reqs.push({ 
                                    ...
                                    count: 1, 
                                    customName: `Диалог NPC (ID: ${task.npcDialogID})`, 
                                    consume: false, 
                                    taskType: 'retrieval', 
                    ...
                    if (q.rewards) {
                        Object.values(q.rewards).forEach(rew => {
                            if (rew.rewardID === 'bq_standard:item' && rew.rewards) {
                                ...
                                rewards.push({ 
                                    item: { item_key: 'command', name: 'Команда', image: 'command_block.png', mod: 'Система' }, 
                                    count: 1, 
                                    customName: 'Команда', 
                                    isChoice: false, 
                                    taskType: 'command', 
                    ...
                    let questIconStr = '';
                    if (q.properties && q.properties.betterquesting && q.properties.betterquesting.icon) {
            ...
            if (newMods.length > 0) {
                editor.activeModId = newMods[0].id;
            } else {
            ...
            alert('Ошибка парсинга файла! Убедитесь, что это правильный QuestDatabase.json');
        }
    },
    ...
    exportData(mods) {
        ...
        // Назначаем числовые ID
        mods.forEach(mod => { 
            mod.quests.forEach(q => { 
                if(idMap[q.id] === undefined) {
                    idMap[q.id] = questNumericId++; 
                }
            }); 
        });
        ...
        mods.forEach(mod => {
            mod.quests.forEach(q => {
                ...
                const createItemsDict = (arr) => {
                    ...
                    arr.forEach((req, idx) => {
                        ...
                        if (!req.item.string_id && sysId.includes(':') && !sysId.match(/[a-zA-Z]/)) {
                            ...
                            sysId = parts[0]; 
                            damage = req.damage !== undefined ? req.damage : (parseInt(parts[1]) || 0);
                        }
                        
                        const dictObj = { "id:8": sysId, "Count:3": parseInt(req.count) || 1, "Damage:2": damage, "OreDict:8": "" };
                        if (req.nbtTag) dictObj["tag:10"] = req.nbtTag;
                        else if (sysId === 'bq_standard:loot_chest') dictObj["tag:10"] = { "hideLootInfo:1": 1 };
                ...
                const createBlocksDict = (arr) => {
                    ...
                    arr.forEach((req, idx) => {
                        ...
                        if (!req.item.string_id && sysId.includes(':') && !sysId.match(/[a-zA-Z]/)) {
                            const parts = sysId.split(':');
                            sysId = parts[0]; 
                            damage = parseInt(parts[1]) || -1;
                        }
                        dict[`${idx}:10`] = { "blockID:8": sysId, "amount:3": parseInt(req.count) || 1, "meta:3": damage, "oreDict:8": "", "nbt:10": req.nbtTag || {} };
                    });
                    return dict;
                };
                ...
                const createFluidsDict = (arr) => {
                    const dict = {};
                    arr.forEach((req, idx) => {
                        let fluidName = req.target || req.customName || "water";
                ...
                if (xps.length > 0) tasks[`${taskIdx++}:10`] = { "taskID:8": "bq_standard:xp", "amount:3": parseInt(xps[0].count) || 1, "isLevels:1": 1, "consume:1": xps[0].consume ? 1 : 0 };
                ...
                npcDialogs.forEach(req => {
                    let dialogId = 0;
                    if (req.nbtTag && req.nbtTag['npcDialogID:3'] !== undefined) {
                        dialogId = req.nbtTag['npcDialogID:3'];
                    }
                ...
                if (q.rewards) {
                    ...
                    q.rewards.forEach(r => {
                        if (r.taskType === 'command') {
                            rewards[`${rewIdx++}:10`] = { "rewardID:8": "bq_standard:command", "command:8": r.command || "", "hideCommand:1": 1, "viaPlayer:1": 0 };
                        } else if (r.taskType === 'xp') {
                            rewards[`${rewIdx++}:10`] = { "rewardID:8": "bq_standard:xp", "amount:3": parseInt(r.count) || 1, "isLevels:1": 1 };
                        }
                    });
                }
