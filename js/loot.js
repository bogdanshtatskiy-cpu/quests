...
export const LootEditor = {
    ...
    init() {
        ...
        document.getElementById('btn-close-loot').addEventListener('click', () => {
            ...
            Editor.lootGroups = {};
            Object.values(this.data['groups:9'] || {}).forEach(g => { Editor.lootGroups[g['ID:3']] = g['name:8']; });
        ...
        document.getElementById('loot-group-name').addEventListener('input', (e) => {
            if(this.activeGroupId !== null) { this.getActiveGroup()['name:8'] = e.target.value; this.renderGroups(); }
        });
        document.getElementById('loot-group-weight').addEventListener('input', (e) => {
            if(this.activeGroupId !== null) { this.getActiveGroup()['weight:3'] = parseInt(e.target.value) || 1; this.renderGroups(); }
        });
    },

    getActiveGroup() { return Object.values(this.data['groups:9']).find(g => g['ID:3'] === this.activeGroupId); },
    ...
    renderGroups() {
        ...
        Object.entries(this.data['groups:9'] || {}).forEach(([key, g]) => {
            ...
            div.addEventListener('click', () => { this.activeGroupId = g['ID:3']; this.renderGroups(); this.renderRewards(); });
            div.querySelector('.danger').addEventListener('click', (e) => {
                ...
                if (confirm('Удалить эту группу лутбокса?')) {
                    ...
                    this.renderGroups(); this.renderRewards();
                }
            });
    ...
    renderRewards() {
            ...
            const itemsHtml = Object.entries(r['items:9'] || {}).map(([iKey, i]) => {
                ...
                return `
                    <div class="loot-item-row">
                        ...
                        <span style="color:#fff; flex:1;">${count}x ${ItemsDB.formatMC(found.name)}</span>
        ...
        list.querySelectorAll('.reward-weight-input').forEach(inp => {
            inp.addEventListener('change', (e) => { group['rewards:9'][e.target.dataset.rkey]['weight:3'] = parseInt(e.target.value) || 1; });
        });
        
        list.querySelectorAll('.btn-del-reward').forEach(btn => {
            btn.addEventListener('click', (e) => { delete group['rewards:9'][e.target.dataset.rkey]; this.renderRewards(); });
        });

        list.querySelectorAll('.btn-del-item').forEach(btn => {
            btn.addEventListener('click', (e) => { delete group['rewards:9'][e.target.dataset.rkey]['items:9'][e.target.dataset.ikey]; this.renderRewards(); });
        });
        ...
        list.querySelectorAll('.btn-add-item').forEach(btn => {
            btn.addEventListener('click', (e) => {
                Editor.openItemPicker((item) => {
                    ...
                    let maxId = -1; Object.keys(r['items:9']).forEach(k => { const n = parseInt(k.split(':')[0]); if(n > maxId) maxId = n; });
                    ...
                    if (!item.string_id && sysId.includes(':') && !sysId.match(/[a-zA-Z]/)) {
                        const parts = sysId.split(':'); sysId = parts[0]; damage = parseInt(parts[1]) || 0;
                    }
                    r['items:9'][newIKey] = { "id:8": sysId, "Count:3": parseInt(count) || 1, "Damage:2": damage, "OreDict:8": "" };
                    this.renderRewards();
                });
            });
        });
    ...
    addGroup() {
        let maxId = -1; Object.values(this.data['groups:9'] || {}).forEach(g => { if(g['ID:3'] > maxId) maxId = g['ID:3']; });
        const newId = maxId + 1;
        let maxKey = -1; Object.keys(this.data['groups:9'] || {}).forEach(k => { const n = parseInt(k.split(':')[0]); if(n > maxKey) maxKey = n; });
        const newKey = (maxKey + 1) + ":10";
        
        this.data['groups:9'][newKey] = { "name:8": "Новый Лутбокс", "weight:3": 1, "ID:3": newId, "rewards:9": {} };
        this.activeGroupId = newId; this.renderGroups(); this.renderRewards();
    },
    ...
    addReward() {
        ...
        let maxId = -1; Object.values(group['rewards:9']).forEach(r => { if(r['ID:3'] > maxId) maxId = r['ID:3']; });
        const newId = maxId + 1;
        let maxKey = -1; Object.keys(group['rewards:9']).forEach(k => { const n = parseInt(k.split(':')[0]); if(n > maxKey) maxKey = n; });
        const newKey = (maxKey + 1) + ":10";
